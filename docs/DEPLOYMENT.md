...
`setup.ps1` 会创建 `.venv`、安装依赖、定位 adb、连接实例、检查 package/Root，并下载或定位 Frida 二进制。它输出的是部署 preflight 结果，不是 Probe READY。若实例 identity、package 或 Root 不符合预期，先修复环境，不修改采集逻辑绕过。

## 3. Root Gate

Root 是 User 控制的实例设置。Collector 脚本只检测 Root，不会自动开启、关闭或回滚。只在专用研究实例由 User 按 [ROOT_TOGGLE.md](ROOT_TOGGLE.md) 开启 Root：

```text
adb -s <serial> shell "/system/xbin/bstk/su -c id"
```

必须返回 `uid=0(root)`。开启前备份配置；Session 后由 User 手动关闭 Root、重启实例，并验证 `su -c id` 不再返回 `uid=0`。
