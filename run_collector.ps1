...
if ($su -notmatch "uid=0") { throw "root NOT active. Enable root on the research instance (docs/ROOT_TOGGLE.md)." }
...
$cleanup.Add("shell $rootLauncher -c 'rm -f $gadgetPath $configPath'")
$cleanup.Add("shell rm -f /data/local/tmp/cf_gadget.so /data/local/tmp/cf_gadget.config.so")
Ok "gadget staged"

Step "3. ADB forward"
& $adb -s $serial forward --remove tcp:$gadgetPort 2>$null | Out-Null
& $adb -s $serial forward tcp:$gadgetPort tcp:$gadgetPort | Out-Null
$cleanup.Add("forward --remove tcp:$gadgetPort")
...
for ($i=0; $i -lt 60; $i++) {
  ...
  $st = Join-Path $sessionDir "state.json"
  if (Test-Path $st) {
    $obj = Get-Content $st -Raw | ConvertFrom-Json
    if ($obj.status -eq "ready") { $ready = $true; break }
  }
}
...
foreach ($c in $cleanup) {
  try {
    if ($c -like "kill-process:*") { Stop-Process -Id ($c -replace 'kill-process:','') -Force -ErrorAction SilentlyContinue }
    elseif ($c -like "forward*") { & $adb -s $serial $c.Split(' ') | Out-Null }
    elseif ($c -like "shell*") { & $adb -s $serial $c.Substring(6) | Out-Null }
  } catch {}
}
